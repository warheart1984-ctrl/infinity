...
def save_adopted_episode(episode: dict[str, Any], *, repo_root: Path | None = None) -> dict[str, Any]:
    ...
    record = {key: value for key, value in episode.items() if key != "cen_approval"}
    refusal = validate_law_state_approval(episode.get("cen_approval"), sink="autobiographical_episode", record=record)
    if refusal:
