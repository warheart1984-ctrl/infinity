...
def save_adopted_habit(habit: dict[str, Any], *, repo_root: Path | None = None) -> dict[str, Any]:
    ...
    record = {key: value for key, value in habit.items() if key != "cen_approval"}
    refusal = validate_law_state_approval(habit.get("cen_approval"), sink="culture_habit", record=record)
    if refusal:
