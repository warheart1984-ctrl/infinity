...
def save_adopted_bond(bond: dict[str, Any], *, repo_root: Path | None = None) -> dict[str, Any]:
    ...
    record = {key: value for key, value in bond.items() if key != "cen_approval"}
    refusal = validate_law_state_approval(bond.get("cen_approval"), sink="social_bond", record=record)
    if refusal:
